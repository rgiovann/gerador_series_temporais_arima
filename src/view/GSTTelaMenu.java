...
/**
 * Classe que constroi a janela de menu do programa e tamb�m estancia as classes
 * relacionadas com cada intem de menu
 *
 * @param - NENHUM
 * @return - NENHUM
 */
public class GSTTelaMenu extends JFrame implements ActionListener, ItemListener, GSTLabels {
    ...
    public GSTTelaMenu() throws IOException
	{
	    ...
	    // *** NOTE ***
	    // Since Java SE 9, invoking getResourceXXX on a class in a named module will
	    // only locate the resource in that module,
	    // it will not search the class path as it did in previous release.
	    //
	    // Create icon image (fundo do frame)
	    // debug
	    // ImageIcon imagemFundoRNA = new
	    // ImageIcon(this.getClass().getResource("correlograma.jpg"));

	    // ImageIcon imagemFundoRNA = new
	    // ImageIcon(this.getClass().getResource("/image/correlograma.jpg"));
	    ...
	    imagemFundoRNA.setImage(imagemFundoRNA.getImage().getScaledInstance(Program.getTelaLargura() - 00,
		    Program.getTelaAltura() - 00, 100));
	    ...
	    this.menu = new JMenu("Time Series");
	    this.menu.setMnemonic(KeyEvent.VK_T);
	    this.menu.setToolTipText("Generate time series");

	    this.menuBar.add(this.menu);

	    // a group of JMenuItems
	    this.menuItem = new JMenuItem(GSTLabels.menuItemGeraST);
	    this.menuItem.setMnemonic(KeyEvent.VK_G); // used constructor instead
	    this.menuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_G, ActionEvent.ALT_MASK));
	    ...
	    this.menuItem = new JMenuItem(GSTLabels.menuItemSair);
	    this.menuItem.setMnemonic(KeyEvent.VK_S); // used constructor instead
	    this.menuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_S, ActionEvent.ALT_MASK));
	    ...
	    this.menu.setToolTipText("About");

	    this.menuItem = new JMenuItem(GSTLabels.menuItemHelp);
	    this.menuItem.setMnemonic(KeyEvent.VK_C); // used constructor instead
	    this.menuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_C, ActionEvent.ALT_MASK));
	    this.menu.add(this.menuItem);
			    ...
			    public void run()
				{
				    try
					{
					    ...
					    Window parentWindow = SwingUtilities
						    .windowForComponent(GSTTelaMenu.this.menuItem);
